...
import { FileText, Zap, FolderOpen, Leaf, Search, Filter, Trash2, Eye, Loader2, Upload } from "lucide-react";
import {
...
export default function Dashboard() {
  ...
  // Build chart data from real documents
  ...
  return (
    <div className="page-container">
      ...
      <motion.div variants={container} initial="hidden" animate="show" className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <motion.div variants={item}>
          <StatCard title="Energy Saved" value={`${(totalTokens * 0.0001).toFixed(1)} kWh`} icon={Leaf} gradient="gradient-success" />
        </motion.div>
      </motion.div>

      {docsOverTime.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass-card p-6">
          <h3 className="section-title mb-4">Documents Over Time</h3>
                ...
                filteredDocs.map((doc) => (
                  <tr key={doc.id} className="border-b border-border/30 last:border-0 hover:bg-accent/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm font-medium text-foreground">
                      {doc.compression_rate ? `${doc.compression_rate}%` : "—"}
                    </td>
                    <td className="px-6 py-4">
                      <Badge variant={doc.status === "analyzed" ? "default" : "secondary"} className="text-xs capitalize">
                        {doc.status}
                      </Badge>
                    </td>
